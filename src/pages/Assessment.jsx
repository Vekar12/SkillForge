      ...
      <div className="max-w-2xl mx-auto px-4 py-6 lg:px-8 lg:py-10">
        ...
        <div className="rounded-3xl p-8 text-center" style={{ background: 'var(--surface-1)', border: '1px solid rgba(255,159,10,0.2)' }}>
          ...
          <div className="rounded-2xl p-4 mb-6 text-left" style={{ background: 'rgba(255,159,10,0.06)', border: '1px solid rgba(255,159,10,0.15)' }}>
            <p className="text-xs font-bold mb-2" style={{ color: 'var(--orange)', letterSpacing: '0.08em' }}>HOW TO GET A FREE KEY</p>
            <ol className="space-y-1.5">
              {['Go to console.groq.com', 'Sign up for free', 'Create an API key', 'Paste it in the header above'].map((s, i) => (
                <li key={i} className="flex gap-2 text-xs" style={{ color: 'var(--text-2)' }}>
                  <span className="w-4 h-4 rounded-full flex items-center justify-center font-bold flex-shrink-0" style={{ background: 'rgba(255,159,10,0.15)', color: 'var(--orange)', fontSize: '10px' }}>{i+1}</span>
                  {s}
                </li>
              ))}
            </ol>
          ...
          <a href="https://console.groq.com" target="_blank" rel="noopener noreferrer"
            ...
            style={{ background: 'var(--orange)', color: '#000', height: '52px', borderRadius: '14px', fontSize: '15px', textDecoration: 'none' }}
          >
            Get Free Groq Key ↗
          </a>
        </div>
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-6 lg:px-8 lg:py-10">
        ...
        <div className="flex flex-col gap-2.5 mb-6">
          <ResultCard emoji="✅" label="WHAT YOU GOT RIGHT" content={result.gotRight} color="var(--green)" />
          <ResultCard emoji="❌" label="NEEDS CORRECTION" content={result.needsCorrection} color="var(--red)" />
          <ResultCard emoji="🔍" label="BLIND SPOTS" content={result.blindSpots} color="var(--orange)" />
          <ResultCard emoji="🇮🇳" label="INDIA-SPECIFIC NOTE" content={result.indiaNote} color="var(--blue)" />
          <ResultCard emoji="📌" label="OPEN POINTS FOR TOMORROW" content={result.openPoints} color="var(--purple)" />
        </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 lg:px-8 lg:py-10">
      ...
      <SectionCard>
        ...
        >
          <p className="text-xs font-bold mb-1" style={{ color: 'var(--blue)' }}>YOUR ANSWER SHOULD LOOK LIKE:</p>
          ...
          {STEPS.map((step, i) => (
            <li key={i} className="flex gap-3 items-start">
              <span
                ...
                style={{ background: 'rgba(10,132,255,0.15)', color: 'var(--blue)' }}
              >
                {i + 1}
              </span>
      ...
      <SectionCard>
        ...
        <a
          ...
          style={{ background: 'var(--border-2)', color: 'var(--purple)', height: '48px', borderRadius: '12px', fontSize: '15px', fontWeight: 600, border: '1px solid rgba(191,90,242,0.2)', textDecoration: 'none' }}
        >
          Open Claude.ai ↗
        </a>
      </SectionCard>
      ...
      <div className="mb-3">
        ...
        <button
          ...
          className="w-full font-semibold transition-all active:scale-[0.98] hover:opacity-90"
          style={{
            background: !feedback.trim() || loading ? 'var(--border-2)' : 'var(--blue)',
